...
training_simple, test_simple = generate_samples(data_simple)  
...
X63 = training_simple.iloc[:, :-1]
X63_test = test_simple.iloc[:, :-1]
y = training_simple.iloc[:, -1]
y_true = X63_test = test_simple.iloc[:, -1]

Xlog = logfeatures(training_simple.iloc[:, :-1], training_simple.iloc[:, :-1].columns)

X_testlog = logfeatures(test_simple.iloc[:, :-1], test_simple.iloc[:, :-1].columns)

predwake(X63, y, X63_test, y_true)
    
L = ['mean_eeg_1', 'std_eeg_1', 'min_eeg_1', 'max_eeg_1', '25%_eeg_1',
